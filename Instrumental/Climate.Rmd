...
pinery_precip$month <- as.factor(pinery_precip$month)

ggplot() + 
  geom_boxplot(data = pinery_precip, aes(x = month, y = total_precip), alpha = 0.7) + 
  theme_bw() +
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 
```
...
### Write Climate Record to CSV
```{r}
#write.csv(pinery_temp_mean, file = "outputs/temperature.csv")
#write.csv(pinery_precip_mean, file = "outputs/precipitation.csv")
```
...
Calculate correlation with London Record
